...
@section('content')
    <div class="mb-4">
        <h1 class="mb-1">{{ __('app.add_employee') }}</h1>
        <p class="text-muted mb-0">{{ __('app.add_employee_description') }}</p>
    </div>
        ...
        <form action="{{ route('employees.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="employee_code" class="form-control" value="{{ old('employee_code', $nextEmployeeCode) }}" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="employment_type" class="form-select" required>
                        ...
                        <option value="monthly" @selected(old('employment_type') === 'monthly')>{{ __('app.monthly_employee') }}</option>
                        <option value="daily" @selected(old('employment_type') === 'daily')>{{ __('app.daily_employee') }}</option>
                    </select>
                </div>

                <div class="col-md-4">
                    <label class="form-label">{{ __('app.hire_date') }}</label>
                    <input type="date" name="hire_date" class="form-control" value="{{ old('hire_date', now()->format('Y-m-d')) }}" required>
                </div>

                <div class="col-md-3">
                    <label class="form-label">{{ __('app.monthly_salary') }}</label>
                    <input type="number" step="0.01" min="0" name="monthly_salary" class="form-control" value="{{ old('monthly_salary', 0) }}">
                </div>

                <div class="col-md-3">
                    <label class="form-label">{{ __('app.daily_wage') }}</label>
                    <input type="number" step="0.01" min="0" name="daily_wage" class="form-control" value="{{ old('daily_wage', 0) }}">
                </div>
