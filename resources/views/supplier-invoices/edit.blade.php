...
@section('content')
    <div class="mb-4">
        <h1 class="mb-1">{{ __('app.edit_supplier_invoice') }}</h1>
        <p class="text-muted mb-0">{{ __('app.edit_supplier_invoice_description') }}</p>
    </div>

    <div class="card content-card p-4">
        <form action="{{ route('supplier-invoices.update', $supplierInvoice) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" min="0" name="amount" class="form-control" value="{{ old('amount', $supplierInvoice->amount) }}" required>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <select name="supplier_id" class="form-select" required>
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" @selected(old('supplier_id', $supplierInvoice->supplier_id) == $supplier->id)>{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="4">{{ old('description', $supplierInvoice->description) }}</textarea>
                </div>
            </div>
