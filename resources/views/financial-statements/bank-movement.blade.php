...
@section('content')
    <div class="mb-4">
        <h1 class="mb-1">{{ __('app.bank_movement_report') }}</h1>
        <p class="text-muted mb-0">{{ __('app.bank_movement_report_description') }}</p>
    </div>

    <div class="card table-card p-4 mb-4">
        <form method="GET" action="{{ route('financial-statements.bank-movement') }}">
            <div class="row g-3">
                <div class="col-md-5">
    ...
    @if(!$bankAccount)
        <div class="alert alert-warning">{{ __('app.bank_account_missing') }}</div>
    @else
