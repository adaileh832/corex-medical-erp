...
<head>
    ...
    <title>Edit Doctor | تعديل طبيب</title>
    <style>
...
<div class="container">
    <div class="topbar">
        <div>
            <h1 style="margin:0;">تعديل طبيب / Edit Doctor</h1>
            <p style="margin:8px 0 0;color:#64748b;">تحديث بيانات الطبيب داخل CoreX</p>
        </div>
        ...
        <form method="POST" action="{{ route('doctors.update', $doctor) }}">
            ...
            <div class="grid">
                <div>
                    ...
                    <input id="full_name" type="text" name="full_name" value="{{ old('full_name', $doctor->display_name) }}" required>
                </div>

                <div>
                    <label for="national_id">الرقم الوطني / National ID</label>
                    <input id="national_id" type="text" name="national_id" maxlength="10" pattern="[0-9]{10}" value="{{ old('national_id', $doctor->national_id) }}" required>
                </div>

                <div>
                    <label for="specialty">التخصص / Specialty</label>
                    <input id="specialty" type="text" name="specialty" value="{{ old('specialty', $doctor->specialty) }}">
                </div>

                <div>
                    <label for="phone">الهاتف / Phone</label>
                    <input id="phone" type="text" name="phone" value="{{ old('phone', $doctor->phone) }}">
                </div>

                <div>
                    <label for="email">البريد الإلكتروني / Email</label>
                    <input id="email" type="email" name="email" value="{{ old('email', $doctor->email) }}">
                </div>

                <div>
                    <label for="license_number">رقم الترخيص / License Number</label>
                    <input id="license_number" type="text" name="license_number" value="{{ old('license_number', $doctor->license_number) }}">
                </div>

                <div class="full">
                    <label for="notes">ملاحظات / Notes</label>
                    <textarea id="notes" name="notes">{{ old('notes', $doctor->notes) }}</textarea>
                </div>
            </div>
